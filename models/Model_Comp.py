...
#               OCED Toolbox, Nexus Meteor, Oasis TIMES, Biotransformer, and SyGMa.
...
#
# Notes: This script uses pandas, numy, and rdkit and their dependecies
#
...
def TIMES_cleanup (file, Model_Module):
    ...
    return df[['DTXSID','Metabolite_INCHIKEY','Clean_SMILES', Model_Module]];
...
def Meteor_cleanup (file):
    ...
    df['Metabolite_INCHIKEY'] = num.NaN
...
def CTS_cleanup(filePath, dictDTXSID):
    ...
    parentInchiKey = parentSMILES.apply(lambda x: Chem.MolToInchiKey(Chem.MolFromSmiles(x)))
...
def SMILES_to_InchiKey (smile_List, stereoisomer = True):
    """Uses RDKit to convert a list of SMILES to a list of InChI keys"""
    ...
    if stereoisomer == False:                                                               #If QSAR Ready InChI Keys are needed, a SMILES without specified stereochemistry
        ...
        InchiList = molList.apply(lambda x: Chem.MolToInchiKey(x))
    else:
        molList = smile_List.apply(lambda x: Chem.MolFromSmiles(x))                         #Uses RDKit to convert SMILES to mols,
        InchiList = molList.apply(lambda x: Chem.MolToInchiKey(x))                          #then mols to InChI keys
    return InchiList;
...
def aggregate_DFs(DF_List, arg_on = ['DTXSID','Metabolite_INCHIKEY'], arg_how = 'outer'):
    ...
    for DF in range(len(comp_DF)):
        ...
        agg_DF = agg_DF.replace(num.NaN, 0)
...
def aggregate_DFs_extended(DF_list, arg_on = ['DTXSID','Metabolite_INCHIKEY'], arg_how = 'outer'):
    ...
    all_smiles.replace(0,num.NaN, inplace = True)
